...
# =====================================================================
# SAFE SQLITE / JSON FALLBACK STORAGE ENGINE
# =====================================================================
HAVE_SQLITE = False
try:
    import sqlite3
    ...
    print("⚠️ 'sqlite3' module not available in this Python build. Using JSON file cache fallback.")
...
def init_db():
    if HAVE_SQLITE:
        try:
            ...
            print(f"⚠️ SQLite Init Failed: {e}. Falling back to JSON file cache.")
...
# =====================================================================
# INITIALIZE CUTLET (ROMAJI) & TRANSLATION ENGINES
# =====================================================================
try:
    import cutlet
    katsu = cutlet.Cutlet()
    ...
    katsu = None
    print(f"⚠️ Cutlet failed to load: {e}.")
...
try:
    ...
    print(f"⚠️ Deep Translator failed to load: {e}.")
...
def translate_english_batch(lines_list):
    ...
    try:
        ...
        print(f"⚠️ English Batch Translation Error: {e}", file=sys.stderr)
        return lines_list
...
def convert():
    try:
        ...
        print(f"⚙️ [TRANSLATING] Processing {len(base_lyrics)} lines -> '{mode}' [{script}] for: {cache_key}")
        ...
        if mode == "english":
            ...
            for line in base_lyrics:
                ...
                if katsu:
                    try:
                        processed_text = katsu.romaji(orig)
                    except Exception as e:
                        print(f"⚠️ Cutlet Error on line '{orig}': {e}", file=sys.stderr)
                        processed_text = orig
                processed_lyrics.append({"time": line.get('time', 0), "text": processed_text})
        ...
        return jsonify({"lyrics": [{"time": 0, "text": f"⚠️ Server Error: {str(e)}"}], "script": "latin"}), 500


if __name__ == '__main__':
    print("🚀 Starting translation engine on http://127.0.0.1:8080")
